...
* 重要：所有坐标 (x, y) 必须是 0.0 到 1.0 之间的归一化值，其中 (0.0, 0.0) 是屏幕左上角，(1.0, 1.0) 是右下角。请务必使用四位小数的精度输出归一化坐标（例如 0.1234, 0.5678），以确保点击位置的精确性。
* 坐标计算示例：如果屏幕分辨率是 1920x1080，你想点击位于像素坐标 (960, 540) 的中心点，你应该输出 x=0.5000, y=0.5000。
* 对于小目标（如按钮、图标），精度至关重要。
* 如果你输出的是绝对像素坐标，请确保它是基于分辨率 {Config.SCREENSHOT_WIDTH}x{Config.SCREENSHOT_HEIGHT} 的。但强烈建议输出归一化坐标。
...
""".strip(),
            ...
            "coordinate": {
                "description": "(x, y): 移动鼠标到的归一化 x (0.0-1.0) 和 y (0.0-1.0) 坐标。请精确到小数点后四位。",
                "type": "array",
            },
            "pixels": {
