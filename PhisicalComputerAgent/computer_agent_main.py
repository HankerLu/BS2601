...
- **屏幕右上角菜单栏布局**（从右到左）：
  * 最右侧：通知中心/时间显示区域
  * 控制中心图标：两个圆形开关/滑块图标（类似⚙️），位于时间左侧
  * Wi-Fi 图标：扇形信号图标📶，位于控制中心左侧
  * 蓝牙图标：蓝牙符号，位于 Wi-Fi 左侧
  * 电池图标：电池形状，位于蓝牙左侧
  
- **控制中心内容**：打开后显示音量、亮度、Wi-Fi、蓝牙等快速控制开关和滑块
  * 音量滑块：通常在控制中心的上半部分，带有喇叭图标🔊
  * 亮度滑块：通常在音量下方，带有太阳图标☀️
  
- **图标识别注意事项**：
  * 仔细观察图标的形状和位置，确保点击正确的目标
  * Wi-Fi 图标（扇形）≠ 控制中心图标（两个圆形开关）
  * 如果任务需要打开控制中心，不要点击 Wi-Fi 图标

**坐标精确性要求**：
- 所有坐标必须使用归一化值（0.0-1.0），精确到小数点后四位
- 对于小图标，坐标偏差 0.01 可能导致点击错误的目标
- 仔细观察截图中图标的实际位置，确保坐标准确

**错误处理与重试策略**：
- **如果操作结果不符合预期**（例如：想打开控制中心却打开了 Wi-Fi）：
  1. 在思考部分明确说明："上一次操作点击了错误的位置/图标"
  2. 重新仔细观察截图，找到正确目标的准确位置
  3. 分析坐标偏差（是偏左了？偏右了？）
  4. 调整坐标后重新尝试（例如：如果点到了 Wi-Fi，控制中心在其右侧，x 坐标应该增大）
  
- **重复失败处理**：
  * 不要连续 3 次点击完全相同的坐标
  * 如果 2 次尝试相同坐标都失败，必须调整策略
  * 考虑使用键盘快捷键作为备选方案
  
- **操作后验证**：
  * 每次操作后，观察屏幕变化是否符合预期
  * 如果屏幕没有预期变化，说明操作失败，需要调整
...
class ComputerAgentWorker(QThread):
    ...
    def __init__(self, user_query, model_id):
        ...
        # 重复操作检测
        self.action_history_tracking = []  # 存储最近的操作历史
            ...
            while self.is_running:
                ...
                try:
                    ...
                    if '<tool_call>' in output_text:
                        ...
                        # ========== 重复操作检测 ==========
                        current_coordinate = action_params.get("coordinate")
                        is_repeat = False
                        
                        # 检查是否与上次操作相同
                        if action_type == self.last_action_type and current_coordinate == self.last_coordinate:
                            self.repeat_count += 1
                            is_repeat = True
                            self.log_signal.emit(f"⚠️ 检测到重复操作 (第 {self.repeat_count} 次)")
                            
                            # 如果重复3次，强制干预
                            if self.repeat_count >= 3:
                                self.log_signal.emit(f"❌ 连续 {self.repeat_count} 次相同操作失败，强制干预")
                                self.log_signal.emit(f"💡 建议：尝试调整坐标或使用不同方法")
                                
                                # 不执行这次重复操作，直接添加干预信息到消息历史
                                intervention_message = f"""
⚠️ 系统干预提示：

你已经连续 {self.repeat_count} 次在坐标 {self.last_coordinate} 执行 {self.last_action_type} 操作，但都没有达到预期效果。

请立即：
1. 承认上次的坐标/方法是错误的
2. 仔细重新观察截图，找到正确的目标位置
3. 如果是点击操作，分析目标图标是在当前坐标的左边还是右边
4. 调整坐标（例如：向左移动减小x值，向右移动增大x值）
5. 或者考虑使用键盘快捷键等替代方法
...
"""
                                ...
                                # 添加干预消息，跳过执行，直接进入下一轮
                                # 这里我们通过修改 prev_action_summary 来传递信息
                                last_action_summary = intervention_message
                                
                                # 短暂等待后继续下一步（不执行当前重复操作）
                                time.sleep(1)
                                continue  # 跳过本次执行，直接进入下一轮
                        else:
                            # 不是重复操作，重置计数
                            self.repeat_count = 0
                        ...
                        self.action_history_tracking.append({
                            "step": step_count,
                            "action": action_type,
                            "coordinate": current_coordinate
                        })
                        # 只保留最近5次操作
                        if len(self.action_history_tracking) > 5:
                            self.action_history_tracking.pop(0)
                        # ========== 重复操作检测结束 ==========
